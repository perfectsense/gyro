...
public abstract class BeamResource<B extends BeamCloud, A> extends Config {
     ...
     *
     * @param cloudConfig Can't be {@code null}.
     */
    public static void updateTree(CloudConfig cloudConfig) {
        updateTreeRecursively(cloudConfig, null);
        updateDependenciesRecursively(cloudConfig);
    }

    private static void updateTreeRecursively(Object object, BeamResource<? extends BeamCloud, ?> parent) {
        try {
                    ...
                    if (value != null) {
                        for (Object item : ObjectUtils.to(Iterable.class, value)) {
                            if (item instanceof BeamResource) {
                                ...
                                BeamResource<? extends BeamCloud, ?> itemResource = itemRef.resolve();
    ...
    public abstract void init(B cloud, BeamResourceFilter filter, A cloudResource);

    public void initAsync(List<CompletableFuture> futures, B cloud, BeamResourceFilter filter, A cloudResource) {
        CompletableFuture future = CompletableFuture.supplyAsync(
                () -> {
                    init(cloud, filter, cloudResource);
                    return this;
                }
        );
    ...
    public void diffOnUpdate(ResourceChange change, BeamResource<B, A> current) throws Exception {
    }
    ...
    public BeamResource<B, A> findCurrent(B cloud, BeamResourceFilter filter) {
        return null;
    }

    public abstract void create(B cloud);

    public abstract void update(B cloud, BeamResource<B, A> current, Set<String> changedProperties);
